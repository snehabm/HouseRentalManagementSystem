 ...
 <div class="container-fluid">
  ...
  <div class="modal fade" id="myModal" role="dialog">
    ...
    <div id="menu4" class="tab-pane fade">
	  ...
	  <table>
	  <tr><th>#</th><th>Name</th><th>Message</th><th>Reply</th></tr>
            <?php 
      $owner_id=$rows['owner_id']; 
      $sql1="SELECT * FROM chat where owner_id='$owner_id' and sender=0";
  ...
  {  
	  ...
	  $i=0;
		...
		if(mysqli_num_rows($query2)>0){
			...
			//echo '<div class="tab"><button class="tablinks" id="defaultOpen" onmouseover="openCity(event, '.$row["m_id"].')">'.$rows["full_name"].'</button></div>';
			echo "<tr><td>".$i."</td><td>".$rows["full_name"]."</td>";
				 
		}
		//echo '<div id="'.$row["m_id"].'" class="tabcontent">'.$row["message"].'</div>';
		echo "<td>".$row["message"]."</td>";
		echo "<td><form method='post' action='owner-index.php'><textarea name='mes'></textarea><input type='submit' value='Reply' name='reply'><input type='hidden' value=".$owner_id." name='o_id'><input type='hidden' value=".$tenant_id." name='t_id'></form></td></tr>";
...
<?php 
	if(isset($_POST['reply'])){
		$owner_id=$_POST['o_id'];
		$tenant_id=$_POST['t_id'];
		$message=$_POST['mes'];
		  $sql="INSERT INTO chat(message,owner_id,tenant_id,sender) VALUES ('$message','$owner_id','$tenant_id',1)";

		  $query = mysqli_query($db,$sql);
		  if($query)
		  {
			echo "<script>window.alert('Repy sent')</script>";
		  }
		  else{
			  echo "<script>window.alert('Repy not sent')</script>";
		  }
...
<div id="map_canvas"></div>
        ...
        <div class="col-sm-6">
          <div class="form-group">
			  ...
			  <input type="text" class="form-control" name="country" required="required" placeholder="Enter Country">
            ...
            <div class="form-group">
			  ...
			  <input type="text" class="form-control" name="province" required="required" placeholder="Enter province/state">
         
            </div>
            <div class="form-group">
              <label for="zone">Zone:</label>
			  <input type="text" class="form-control" name="zone" required="required" placeholder="Enter zone">
             
            </div>
            <div class="form-group">
              <label for="district">District:</label>
			  <input type="text" class="form-control" name="district" required="required" placeholder="Enter district">
...
<?php $sql2="SELECT * from property_photo where property_id='$property_id'";
	  ...
	  <?php }}?>
                ...
                <form method="POST" action="owner-index.php">
                <td>
				 ...
				 <!--<a href="#menu7" class="btn btn-danger" name="delete_property" value="Delete">Delete</a>-->
...
<?php
			...
			
		{
			...
			if($result=mysqli_query($db,$sql)){
				echo "<script>window.alert('the property with id :$pid was deleted sucessfully')</script>";
			}
			else{
	...
	<div id="menu5" class="tab-pane fade">
      ...
      <div class="container">
        ...
        <div class="col-sm-6">
          <div class="form-group">
              <label for="country">Country:</label>
			  <input type="text" class="form-control" name="country" required="required" value=<?php echo $rows['country'] ?> placeholder="Enter Country">
             <!--<select  name="country" required="required">
                               <option value="">--Select Country--</option>
                                <option value="Nepal">Nepal</option>
              </select>-->
            ...
            <div class="form-group">
              <label for="province">Province/State:</label>
			  <input type="text" class="form-control" name="province" required="required" value=<?php echo $rows['province'] ?> placeholder="Enter province/state">
          
            </div>
            <div class="form-group">
              <label for="zone">Zone:</label>
			  <input type="text" class="form-control" name="zone" required="required" value=<?php echo $rows['zone'] ?> placeholder="Enter zone">
             </option>
                            </select>
            </div>
            <div class="form-group">
              <label for="district">District:</label>
			  <input type="text" class="form-control" name="district" required="required" value=<?php echo $rows['district'] ?> placeholder="Enter district">
            ...
            <div class="form-group">
              <label for="vdc/municipality">Municipality:</label>
              <select class="form-control" value="Municipality"  name="vdc_municipality">
                <option value="">--Select</option>
        ...
        <div class="col-sm-6">
                  ...
                  <div class="form-group">
                    <label for="description">Full Description:</label>
                    <textarea type="text" class="form-control" placeholder="desc" id="description" value=<?php echo $rows['description']; ?> name="description"></textarea>
                  </div>
                  <table class="table table-bordered" border="0">  
                  <tr> 
                    <div class="form-group"> 
                    <label><b>Latitude/Longitude:</b><span style="color:red; font-size: 10px;">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; *Click on Button</span></label>                    
                    <td><input type="text" name="latitude" placeholder="Latitude" value=<?php echo $rows['latitude'] ?> id="latitude" class="form-control name_list" readonly required /></td>
                    <td><input type="text" name="longitude" placeholder="Longitude" value=<?php echo $rows['longitude'] ?> id="longitude" class="form-control name_list" readonly required /></td> 
                    <td><input type="button" value="Get Latitude and Longitude" onclick="getLocation()" class="btn btn-success col-lg-12"></td>  
                  </div>
                  </tr>  
                </table>
